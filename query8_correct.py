...
# Query 8: Freshness check - using correct column names from schema
cur.execute("""
    SELECT (SELECT MAX(created_at) FROM signals) AS last_signal_ts, 
           (SELECT MAX(collected_at) FROM funding_rates) AS last_funding_ts, 
           (SELECT MAX(collected_at) FROM open_interest) AS last_oi_ts, 
           (SELECT MAX(collected_at) FROM ls_ratios) AS last_ls_ts, 
