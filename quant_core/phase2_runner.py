...
from config import PAIRS, PHASE2_VALIDATION_END_TIME_MS, PHASE2_VALIDATION_15M_CANDLES, PHASE2_VALIDATION_4H_CANDLES
...
async def run():
    ...
    for r, stats in regime_stats.items():
        w, l = stats["wins"], stats["losses"]  # noqa: E741
        expected_regime_pf = (w * 3.0) / l if l > 0 else float('inf')
        actual_regime_pf = (w * 3.0) / l if l > 0 else float('inf')
